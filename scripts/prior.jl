...
for i=1:100
    ...
    p1 = bar(Beluga.freqmap(chain.trace[50:end,:k]))
    plot!(0:kmax, pdf(prior.πK, 0:kmax), linewidth=2)
    p2 = histogram(log.(chain.trace[50:end,:λ1]), normalize=true)
    plot!(p2, Normal(prior.X₀.μ[1], sqrt(prior.X₀.Σ.mat[1,1])), linewidth=2)
    p3 = histogram(log.(chain.trace[50:end,:μ1]), normalize=true)
    plot!(p3, Normal(prior.X₀.μ[2], sqrt(prior.X₀.Σ.mat[2,2])), linewidth=2)
    p4 = histogram(chain.trace[50:end,:η1], normalize=true)
    plot!(p4, prior.πη, linewidth=2)
