...
df3 = CSV.read("test/data/plants1-100.tsv", delim=",")

s1 = "(D:18.03,(C:12.06,(B:7.06,A:7.06):4.99):5.97);"
s3 = open("test/data/plants1.nw", "r") do f ; readline(f); end
...
shouldbe3 = [-1296.1763022581217, -1408.4459849625102, -1172.356609929616, -1197.7833487345943, -3849.887791929266, -1348.7468163817248, -1486.4818702521393, -2083.8730465634676, -2351.3561862674096, -1364.8837334043392]
...
@testset "DL model, full loglikelihood" begin
    ...
    for i=1:length(λ)
        d, y = DuplicationLossWGDModel(s3, df3, λ[i], μ[i], η[i])
        p = Profile(y)
        l = logpdf!(d, p)
        @test shouldbe3[i] ≈ l
    end
