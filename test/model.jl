...
df = DataFrame(:A=>[2,3],:B=>[2,5],:C=>[3,0],:D=>[4,1])
...
@testset "DL model, Csuros & Miklos algorithm" begin
    d, y = DuplicationLossWGDModel(s, df, 0.2, 0.3, 1/1.5)
    x = y[:,1]
...
@testset "Stress test" begin
    ...
    for i=1:100
        ...
        d, y = DuplicationLossWGDModel(s, df, λ, μ, η)
        logpdf(d, y[:,1])
    end
    r1 = Normal(0, 5)
    for i=1:100
        λ, η = exp(rand(r1)), rand(r2)
        d, y = DuplicationLossWGDModel(s, df, λ, λ, η)
        logpdf(d, y[:,1])
    end
end
...
#=
Previous implementation benchmark (computation from scratch)
